...
@section('content')
    @php
       $price = $productAttributes->count() ? $productAttributes[0]->variants[0]->price : null;
    ...
    <section class="section section-xxl section-first bg-default">
        <div class="container">
            <div class="row row-30">
                ...
                <div class="col-lg-6">
                    <form class="single-product" id="single-product">
                        <h3 class="text-transform-none font-weight-medium">{{$product->translate($current_language)->name}}</h3>
                        <div class="group-md group-middle">
                            <div class="single-product-price" id="single-product-price">{{ $single_product_attribute ? $single_product_attribute->price : ($price ?? $product->price)}} AZN</div>
                            ...
                            <ul class="list list-description">
                                @foreach($productAttributes as $productAttribute)
                                    <li>
                                        ...
                                        <span>
                                            <select name="single_product_attribute_id" id="single_product_attribute_id">
                                                @foreach($productAttribute->variant_products($product['id']) as $variant)
                                                    <option {{ request('single_product_attribute_id') == $variant['id'] ? 'selected' : '' }} title="{{ $variant->price }}" value="{{ $variant->id }}">{{ $variant->name }}</option>
                                                @endforeach
                                            </select>
                                        </span>
                                    </li>
                        ...
                        <input type="hidden" name="attribute_id" id="attribute_id" value="{{ request('single_product_attribute_id') }}">
                        <div class="group-xs group-middle">
                            <div class="product-stepper">
                                <label>
                                    <input class="form-input qyt_{{$product->id}}" name="quantity" type="number" data-zeros="true" value="1" min="1" max="1000">
                                </label>
                            </div>
                            <div>
                                <button type="button" class="button button-lg button-secondary button-zakaria add-to-cart-kubpro" data-id="{{$product->id}}" data-type="1">@lang('Add to cart')</button>
                            </div>
                        </div>
                        <hr class="hr-gray-100">
                        <div class="group-xs group-middle"><span class="list-social-title">@lang('Share')</span>
                            <div>
                        ...
                        <div class="col-sm-6 col-md-4 col-lg-3">
                            <article class="product wow fadeInRight">
                                ...
                                <div class="product-button-wrap">
                                    ...
                                    <div class="product-button">
                                        <button class="button button-primary-2 button-zakaria fl-bigmug-line-shopping202 add-to-cart-kubpro" data-id="{{$featuredProduct->id}}" data-type="2"> </button>
                                    </div>
                                </div>
                            </article>
                        </div>
...
@push('footer')
    <script>
        $('#single_product_attribute_id').on('change', function (){
            var price = $(this).find("option:selected").attr("title");
            $('#single-product-price').html(price+' AZN');
            let single_product = $('#single-product').serialize();
